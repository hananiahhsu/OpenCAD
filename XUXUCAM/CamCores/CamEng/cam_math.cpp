#include "CamCores/CamEng/cam_math.h"
...
#include <cmath>
#include "CamThirdParty/muparser/include/muParser.h"
#include <QString>
#include <QDebug>
 ...
 * Save pow function
 */
double Cam_Math::pow(double x, double y) {
    errno = 0;
    double ret = ::pow(x, y);
    if (errno==EDOM) {
        RS_DEBUG->print(RS_Debug::D_ERROR,
                        "Cam_Math::pow: EDOM in pow");
        ret = 0.0;
    }
    else if (errno==ERANGE) {
        RS_DEBUG->print(RS_Debug::D_WARNING,
                        "Cam_Math::pow: ERANGE in pow");
        ret = 0.0;
    }
    return ret;
}
...
RS_Vector Cam_Math::pow(RS_Vector vp, double y) {
 ...
 */
double Cam_Math::rad2deg(double a) {
    return 180./M_PI*a;
}
 ...
 */
double Cam_Math::deg2rad(double a) {
    return M_PI/180.0*a;
}
 ...
 */
double Cam_Math::rad2gra(double a) {
    return 200./M_PI*a;
}

double Cam_Math::gra2rad(double a) {
    return M_PI/200.*a;
}
 ...
 */
unsigned Cam_Math::findGCD(unsigned a, unsigned b) {
 ...
 */
bool Cam_Math::isAngleBetween(double a,
                             double a1, double a2,
                             bool reversed) {
 ...
 */
double Cam_Math::correctAngle(double a) {
    return M_PI + remainder(a - M_PI, m_piX2);
}

double Cam_Math::correctAngleU(double a) {
 ...
 */
double Cam_Math::getAngleDifference(double a1, double a2, bool reversed) {
    if(reversed) std::swap(a1, a2);
    return correctAngle(a2 - a1);
}

double Cam_Math::getAngleDifferenceU(double a1, double a2)
{
 ...
 */
double Cam_Math::makeAngleReadable(double angle, bool readable,
                                  bool* corrected) {
 ...
 */
bool Cam_Math::isAngleReadable(double angle) {
 ...
 */
bool Cam_Math::isSameDirection(double dir1, double dir2, double tol) {
 ...
 */
double Cam_Math::eval(const QString& expr, double def) {

    bool ok;
    double res = Cam_Math::eval(expr, &ok);

    if (!ok) {
        //std::cerr << "Cam_Math::evaluate: Parse error at col "
        //<< ret << ": " << fp.ErrorMsg() << "\n";
        return def;
    }
 ...
 */
double Cam_Math::eval(const QString& expr, bool* ok) {
    bool okTmp(false);
    if(!ok) ok=&okTmp;
    if (expr.isEmpty()) {
        *ok = false;
 ...
 */
QString Cam_Math::doubleToString(double value, double prec) {
    if (prec< RS_TOLERANCE ) {
        RS_DEBUG->print(RS_Debug::D_ERROR,
                        "Cam_Math::doubleToString: invalid precision");
        return QString().setNum(value, prec);
    }

    double const num = Cam_Math::round(value / prec)*prec;

    QString exaStr = Cam_Math::doubleToString(1./prec, 10);
    int const dotPos = exaStr.indexOf('.');

    if (dotPos==-1) {
        //big numbers for the precision
        return QString().setNum(Cam_Math::round(num));
    } else {
        //number of digits after the point
        int digits = dotPos - 1;
        return Cam_Math::doubleToString(num, digits);
    }
}
 ...
 */
QString Cam_Math::doubleToString(double value, int prec) {
    QString valStr;
...
void Cam_Math::test() {
    ...
    std::cout << "Cam_Math::test: doubleToString:\n";

    v = 0.1;
    s = Cam_Math::doubleToString(v, 0.1);
    assert(s=="0.1");
    s = Cam_Math::doubleToString(v, 0.01);
    assert(s=="0.10");
    ...
    s = Cam_Math::doubleToString(v, 0.01);
    assert(s=="0.01");
    s = Cam_Math::doubleToString(v, 0.001);
    assert(s=="0.010");
    ...
    std::cout << "Cam_Math::test: complete"<<std::endl;
}
...
std::vector<double> Cam_Math::quadraticSolver(const std::vector<double>& ce)
...
std::vector<double> Cam_Math::cubicSolver(const std::vector<double>& ce)
...
**/
std::vector<double> Cam_Math::quarticSolver(const std::vector<double>& ce)
{
    std::vector<double> ans(0,0.);
...
**/
std::vector<double> Cam_Math::quarticSolverFull(const std::vector<double>& ce)
{
    if(RS_DEBUG->getLevel()>=RS_Debug::D_INFORMATIONAL){
        DEBUG_HEADER
    ...
    if ( fabs(ce[4]) < 1.0e-14) { // this should not happen
        if ( fabs(ce[3]) < 1.0e-14) { // this should not happen
            if ( fabs(ce[2]) < 1.0e-14) { // this should not happen
                ...
                roots=Cam_Math::quadraticSolver(ce2);
            }
        } else {
            ce2.resize(3);
            ce2[0]=ce[2]/ce[3];
            ...
            roots=Cam_Math::cubicSolver(ce2);
        }
    } else {
        ce2[0]=ce[3]/ce[4];
        ce2[1]=ce[2]/ce[4];
        ...
        if(fabs(ce2[3])<= RS_TOLERANCE15) {
            ...
            roots=Cam_Math::quarticSolver(ce2);
    }
    return roots;
}
...
bool Cam_Math::linearSolver(const std::vector<std::vector<double> >& mt, std::vector<double>& sn){
 ...
 * @author: Dongxu Li
 */
double Cam_Math::ellipticIntegral_2(const double& k, const double& phi)
{
...
/* solve the following quadratic simultaneous equations,
  ...
  *@return a vector contains real roots
  */
RS_VectorSolutions Cam_Math::simultaneousQuadraticSolver(const std::vector<double>& m)
{
...
/* solve the following quadratic simultaneous equations,
  ...
  */
RS_VectorSolutions Cam_Math::simultaneousQuadraticSolverFull(const std::vector<std::vector<double> >& m)
{
...
RS_VectorSolutions Cam_Math::simultaneousQuadraticSolverMixed(const std::vector<std::vector<double> >& m)
{
    RS_VectorSolutions ret;
    auto p0=& (m[0]);
    auto p1=& (m[1]);
    ...
    if(p1->size()==3) {
            ...
            ce[0][2]=-ce[0][2];
            ce[1][2]=-ce[1][2];
            if( Cam_Math::linearSolver(ce,sn)) ret.push_back(RS_Vector(sn[0],sn[1]));
            return ret;
    }
//    DEBUG_HEADER
...
/** verify a solution for simultaneousQuadratic
  ...
  **/
bool Cam_Math::simultaneousQuadraticVerify(const std::vector<std::vector<double> >& m, RS_Vector& v)
{
    RS_Vector v0=v;
    auto& a=m[0][0];
    auto& b=m[0][1];
